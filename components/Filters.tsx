...
const Filters: React.FC<FiltersProps> = ({
  ...
  const handleSuspensionChange = (suspensionType: string) => {
    const newTypes = filters.suspensionTypes.includes(suspensionType)
      ? filters.suspensionTypes.filter((s) => s !== suspensionType)
      : [...filters.suspensionTypes, suspensionType];
    onFilterChange("suspensionTypes", newTypes);
  };
  ...
  return (
    <div className="bg-white p-5 rounded-xl border border-slate-200">
      ...
      <AccordionItem title="Тип підвіски">
          ...
          {allSuspensionTypes.map((type) => (
            ...
            >
              <input
                ...
                onChange={() => handleSuspensionChange(type)}
                className="h-4 w-4 rounded border-slate-300 text-orange-600 focus:ring-orange-500"
              />
