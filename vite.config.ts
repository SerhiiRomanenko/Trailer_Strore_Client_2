...
const REDIS_URL = process.env.REDIS_URL; // наприклад: redis://default:PASS@steady-albacore-15146.upstash.io:6379
...
// -------------------- REDIS (STABLE for Upstash) --------------------
if (REDIS_URL) {
    // Базові опції для serverless Upstash
    const redisOptions = {
        maxRetriesPerRequest: null,   // дозволяємо ioredis робити власні retry-и
        enableReadyCheck: false,      // вимикаємо ready check — підходить для serverless
        reconnectOnError: () => true, // дозволяємо reconnect
        lazyConnect: true,            // не викликаємо connect() вручну — авто-підключення при першій команді
    };

    // Якщо хост Upstash — явно додаємо tls (Upstash часто вимагає TLS)
...
async function start() {
    ...
    try {
        ...
        if (!(await clientTG.isUserAuthorized())) {
            ...
            // НЕ робимо автоматичну авторизацію у production, але залишаємо можливість
            // await authorize();
        } else {
